...
const useFormCounter = (
  ...
  const [uncompletedFieldsArray, setUncompletedFieldsArray] = useState<(keyof TEmployeeSettings)[]>([]);

  const returnErrorStyles = useCallback(
    (field: keyof TEmployeeSettings) => {
      if (
  ...
  const updateTotalCompletedFields = () => {
    ...
    const uncompletedFields: (keyof TEmployeeSettings)[] = [];
